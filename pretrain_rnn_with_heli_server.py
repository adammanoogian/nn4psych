...
from utils_funcs import plot_analysis, get_lrs, saveload
...
exptname = f"{reward_size}rz_{hidden_dim}n_{gamma}g_{max_displacement}md_{seed}s"
print(exptname)
...
plt.subplot(231)
plt.plot(np.mean(epoch_G,axis=2))
plt.xlabel('Epoch')
plt.ylabel('G') # should become more positive.

plt.subplot(232)
plt.plot(np.mean(epoch_time,axis=2))
plt.xlabel('Epoch')
plt.ylabel('Time') # should become more positive.

plt.subplot(233)
scores = all_scores[:,0] - all_scores[:,1]
slope, intercept, r_value, p_value, std_err = linregress(np.arange(n_epochs), scores)
plt.plot(scores)
plt.plot(np.arange(n_epochs), slope*np.arange(n_epochs)+intercept, color='k')
plt.xlabel('Epoch')
plt.ylabel('A(CP) - A(OB)') # should become more positive.
...
saveload(f'./best_data/{exptname}', [epoch_G, epoch_time, all_states, ],'save')