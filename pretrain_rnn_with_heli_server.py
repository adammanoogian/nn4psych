...
parser.add_argument('--epochs', type=int, required=False, help='epochs', default=2000)
...
from utils import plot_analysis, get_lrs, saveload
...
train_epochs = n_epochs*0.75 #n_epochs*0.5  # number of epochs where the helicopter is shown to the agent. if 0, helicopter is never shown.
...
for epoch in range(n_epochs):
    ...
    for tt in idxs:
        ...
        _, lrs = get_lrs(all_states[epoch, tt])
...
lrs = np.zeros([2, n_trials-1])

maxstate = all_states[-1].max()
for c in range(2):
    states = all_states[-1,c]
    ...
    lrs[c] = np.where(prediction_error[:-1] != 0, update / prediction_error[:-1], 0)

score = np.mean(lrs[0]) -np.mean(lrs[1])

print(np.mean(lrs,axis=1),np.mean(lrs,axis=1), score)
...
if score >0.5:
    ...
    saveload(f'./best_data/{score}_{exptname}', [epoch_perf, all_states],'save')
