...
def compute_gae(buffer, gamma, device):
    ...
    advantages = torch.stack(
        [a.detach() if isinstance(a, torch.Tensor) else torch.tensor(a) for a in advantages]
    ).to(device)
    returns = advantages + torch.stack(
        [v.detach() if isinstance(v, torch.Tensor) else torch.tensor(v) for v in values]
    ).to(device)
...
def main():
    ...
    # Env parameters
    n_epochs = args.epochs  # number of epochs to train the model on.
    n_trials = args.trials  # number of trials per epoch for each condition.

    train_epochs = n_epochs * args.ratio
    ...
    for epoch in range(n_epochs):
        ...
        idxs = np.random.choice(np.arange(2), size=2, replace=False)
        ...
        for tt in idxs:
            ...
            totG, totloss, tottime = train_with_rollouts(
                env, model, optimizer, epoch=epoch, n_trials=n_trials, gamma=gamma,
