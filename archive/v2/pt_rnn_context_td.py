...
def train(env, model, optimizer, n_trials=500, gamma=0.99):
    ...
    for _ in range(n_trials):
        ...
        # _, last_critic_value, _ = model(state, hx)
        last_critic_value = 0#last_critic_value.detach().squeeze()

        # Calculate temporal difference errors
        # print(rewards.shape, next_values.shape, values.shape)
        rewards = torch.tensor(rewards, dtype=torch.float32)
        values = torch.tensor(values, dtype=torch.float32)
        next_values = torch.cat([v.unsqueeze(0) for v in values[1:]] + [last_critic_value.unsqueeze(0)])
        values = torch.cat([v.unsqueeze(0) for v in values])
        tdes = rewards + gamma * next_values - values

        tdes = torch.tensor(tdes)
        log_probs = torch.stack(log_probs)

        # Compute advantages
        advantages = tdes.detach()

        # Calculate loss
        actor_loss = -(log_probs * advantages).mean()
        critic_loss = (tdes**2).mean()
