...
# Model Parameters
input_dim = 4  # set this based on your observation space
hidden_dim = 128
...
def train(env, model, optimizer, n_trials=500, gamma=0.99):
    ...
    for _ in range(n_trials):
        obs, done = env.reset()
        obs = torch.FloatTensor(obs).unsqueeze(0).unsqueeze(0)  # add batch and seq dim
        ...
        while not done:
            # Forward pass
            actor_logits, critic_value, hx = model(obs, hx)
            ...
            obs = torch.FloatTensor(next_obs).unsqueeze(0).unsqueeze(0)
